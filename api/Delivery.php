...
class Delivery extends Simpla
{
	...
	public function get_delivery($id)
	{
		...
		$query = $this->db->placehold("SELECT id, name, image, description, free_from, price, enabled, position, separate_payment FROM __delivery WHERE id=? LIMIT 1", intval($id));
	...
	{	
		...
		$query = "SELECT id, name, image, description, free_from, price, enabled, position, separate_payment
	...
	{
		$id = (array) $id;
		$query = $this->db->placehold("SELECT image FROM __delivery WHERE id in(?@)", $id);
		$this->db->query($query);
		$filenames = $this->db->results('image');
		if(!empty($filenames))
		{
			$query = $this->db->placehold("UPDATE __delivery SET image=NULL WHERE id in(?@)", $id);
			$this->db->query($query);
			foreach($filenames as $filename)
			{
				$query = $this->db->placehold("SELECT count(*) as count FROM __delivery WHERE image=?", $filename);
				$this->db->query($query);
