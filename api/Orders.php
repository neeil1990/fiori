...
class Orders extends Simpla
{
	...
	public function get_order($id)
	{
		...
		$query = $this->db->placehold("SELECT  o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
										...
										o.date, o.user_id, o.name, o.address, o.phone, o.email, o.comment, o.zedate, o.zetime, o.status,
	...
	function get_orders($filter = array())
	{
		...
		if(isset($filter['url']))
			$url_filter = $this->db->placehold('AND o.url in(?@)', (array)$filter['url']);
		...
		$query = $this->db->placehold("SELECT o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
									...
									WHERE 1
									$id_filter $status_filter $url_filter $user_filter $keyword_filter $label_filter $modified_since_filter GROUP BY o.id ORDER BY status, id DESC $sql_limit", "%Y-%m-%d");
		$this->db->query($query);
		$orders = array();
	...
	{
		$order = (object)$order;
		$order->url = substr(md5(uniqid($this->config->salt, true)), 0, 8);
		$set_curr_date = '';
		if(empty($order->date))
	...
	*/	
	...
	public function add_purchase($purchase)
	{
		...
		$price = 0;
		if( !empty( $_SESSION['shopping_properties'][$purchase->variant_id] ) ) {
			foreach( $_SESSION['shopping_properties'][$purchase->variant_id] as $i ) 
				$variant->price = $variant->price += $i['price'];
		}
