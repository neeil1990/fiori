...
class Comments extends Simpla
{
	...
	{
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.name, c.image, c.email, c.ip, c.type, c.text, c.otvet, c.rate, c.date, c.approved FROM __comments c WHERE id=? LIMIT 1", intval($id));
