		...
		<div id="list" class="sortable">
			...
			<div class="{if !$comment->approved}unapproved{/if} row">
				...
				<div class="name cell">
					...
					{if $comment->email}<a href="mailto:{$comment->email|escape}">{$comment->name|escape}</a>{else}{$comment->name|escape}{/if}
					...
					к новости <a target="_blank" href="{$config->root_url}/blog/{$comment->post->url}#comment_{$comment->id}">{$comment->post->name}</a>
...
<div id="right_menu">
	...
	<ul>
		...
		<li {if $type == 'article'}class="selected"{/if}><a href='{url keyword=null type=article}'>К фотоальбомам</a></li>
