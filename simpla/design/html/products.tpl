...
<div id="right_menu">
	...
	<!-- Бренды -->
	<ul>
		<li {if !$brand->id}class="selected"{/if}><a href="{url brand_id=null}">Все цветы</a></li>
		{foreach $brands as $b}
	...
	<!-- Бренды -->
	<ul>
		<li {if !$whom->id}class="selected"{/if}><a href="{url whom_id=null}">Все равно кому</a></li>
		{foreach $whoms as $b}
		<li whom_id="{$b->id}" {if $whom->id == $b->id}class="selected"{else}class="droppable whom"{/if}><a href="{url keyword=null page=null whom_id=$b->id}">{$b->name}</a></li>
		{/foreach}
	</ul>
	<!-- Бренды (The End) -->
	{/if}
	...
	<!-- Бренды -->
	<ul>
		<li {if !$event->id}class="selected"{/if}><a href="{url event_id=null}">Любое событие</a></li>
		{foreach $events as $b}
		<li event_id="{$b->id}" {if $event->id == $b->id}class="selected"{else}class="droppable event"{/if}><a href="{url keyword=null page=null event_id=$b->id}">{$b->name}</a></li>
		{/foreach}
	</ul>
	<!-- Бренды (The End) -->
	{/if}
