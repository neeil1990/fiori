...
class CartView extends View
{
  ...
  public function __construct()
  {
    ...
    {
		$this->cart->add_item($variant_id, $this->request->get('amount', 'integer'), $this->request->get('properties'));
	    header('location: '.$this->config->root_url.'/cart/');
		
    }

    // Удаление товара из корзины
    if($delete_variant_id = $this->request->get('delete_variant'))
    {
    ...
    if(isset($_POST['checkout']))
    {
		...
		$order->zedate      = $this->request->post('zedate');
		$order->zetime      = $this->request->post('zetime');
    	...
    	else
    	{
	    	...
	    	{
				$properties_text = '';
				if( $properties = $_SESSION['shopping_properties'][$variant_id] ) {
					foreach( $properties as $property )
						$properties_text[] = $property['name'];
				}
				if( $properties_text ) $properties_text = implode( '|||', $properties_text );
				
	    		$this->orders->add_purchase(array('order_id'=>$order_id, 'variant_id'=>$variant_id, 'amount'=>intval($amount), 'properties'=> $properties_text ));
	    	}
